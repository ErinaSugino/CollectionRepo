...
function onNodeConnectionChange(args)
  ...
  if object.isInputNodeConnected(1) then
    onLockedNodeChange(object.getInputNodeLevel(1))
  else
    unlockDoor() --Fallback unlock if a locked door has its wire disconnected
  end
  if object.isInputNodeConnected(0) then
    onOpenNodeChange(object.getInputNodeLevel(0))
  end
end

function onInputNodeChange(args)
  local id = args.node or 0
  local level = args.level or false
  if id == 0 then onOpenNodeChange(level)
  elseif id == 1 then onLockedNodeChange(level) end
end

function onOpenNodeChange(level)
  if level and not storage.locked then
...
function onLockedNodeChange(level)
  if level then
...
function updateInteractive()
  sb.logInfo("Updating interactive state"..tostring(config.getParameter("interactive", true) and not self.sensorConfig and not object.isInputNodeConnected(0) and not storage.locked))
...
function unlockDoor()
  if storage.locked then
    ...
    if object.getInputNodeLevel(0) then openDoor(storage.doorDirection) end
    return true
  end
end
